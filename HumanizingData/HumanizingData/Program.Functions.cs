...
partial class Program
{
    ...
    private static void OutputSpacingAndDashes()
    {
        ...
        WriteLine(
            "Transformed (lower case, then sentence case): {0}",
            ugly.Transform(To.LowerCase, To.SentenceCase)
        );

        WriteLine(
            "Humanized, Transformed (lower case, then sentence case): {0}",
    ...
    private static void NumberFormatting()
    {
        ...
        WriteLine(
            "Original: {0}, Metric: About {1}",
            thousands,
            thousands.ToMetric(
                MetricNumeralFormats.WithSpace | MetricNumeralFormats.UseShortScaleWord,
                decimals: 0
            )
        );
    ...
    private static void DateTimeFormatting()
    {
        ...
        WriteLine(
            "Add 3 hours and 1 minute, Humanized: {0}",
        ...
        WriteLine(
            "Add 7 days and 1 minute, Humanized: {0}",
        ...
        foreach (int days in daysArray)
        {
            ...
            WriteLine(
                "{0} days, Humanized with precision 2: {1}",
                days,
                TimeSpan.FromDays(days).Humanize(precision: 2)
            );

            WriteLine(
                "{0} days, Humanized with max unit days: {1}",
                days,
                TimeSpan.FromDays(days).Humanize(maxUnit: Humanizer.Localisation.TimeUnit.Day)
            );
