...
public partial class CustomersPage : ContentPage
{
    public CustomersPage()
    {
        ...
        try
        {
            ...
            HttpClient client = new() { BaseAddress = new Uri($"http://{domain}:5162") };
            ...
            foreach (
                CustomerDetailViewModel c in customersFromService.OrderBy(
    ...
    async void Customer_Phoned(object sender, EventArgs e)
    {
        ...
        )
        {
            ...
            catch (Exception ex)
            {
                await DisplayAlert(
                    title: "Failed",
                    message: string.Format("Failed to dial {0} due to: {1}", c.Phone, ex.Message),
                    cancel: "OK"
                );
            }
        }
    ...
    {
        await Navigation.PushAsync(
            new CustomerDetailPage(BindingContext as CustomersListViewModel)
        );
    }
}
